...
confile = {:Debian => '/etc/defaults/couchpotato', :RedHat => '/etc/sysconfig/couchpotato' , :Suse => '/etc/sysconfig/couchpotato' }
  ...
  context 'supported operating systems' do
    ['Debian', 'RedHat', 'Suse'].each do |osfamily|
      describe "without any parameters on #{osfamily}" do
        ...
        it { is_expected.to contain_file('/var/lib/couchpotato/.config/.couchpotato.ini') }
        it { is_expected.to contain_file('/var/lib/couchpotato/.config') }
        ...
        if osfamily == 'Debian'
          ...
          it { is_expected.to contain_couchpotato__repo__ppa('ppa:jcfp/ppa') }
        end
        if osfamily == 'RedHat'
          ...
          it { is_expected.to contain_yumrepo('https__dl.dropboxusercontent.com_u_14500830_CouchPotato_RHEL-CentOS_6') }
          it { is_expected.to contain_couchpotato__repo__yum('http://dl.fedoraproject.org/pub/epel/6/x68_64') }
          it { is_expected.to contain_couchpotato__repo__yum('http://packages.atrpms.net/dist/el6/unrar/') }
          it { is_expected.to contain_couchpotato__repo__yum('https://dl.dropboxusercontent.com/u/14500830/CouchPotato/RHEL-CentOS/6') }
        end
        if osfamily == 'Suse'
          it { is_expected.to contain_couchpotato__repo__zyp('http://download.opensuse.org/repositories/Archiving/SLE_12') }
